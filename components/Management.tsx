...
import { 
    ...
    X, Edit3, Save, Sliders, Target, Star, Lock, UserPlus, Fingerprint, Power, Eye
} from 'lucide-react';
...
export const Management: React.FC<ManagementProps> = ({ lang, onDataChange, currentUser, onViewProject }) => {
  ...
  const handleEditUser = (user: User) => {
    ...
    toast.info(lang === 'es' ? `Modificando a ${user.name}` : `Modifying ${user.name}`);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto pb-32 space-y-8 animate-fade-in">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[3rem] shadow-2xl overflow-hidden min-h-[600px]">
            ...
            <div className="p-10 space-y-12 animate-fade-in-up">
                <div className="max-w-4xl bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl mx-auto">
                    <div className="flex items-center gap-4 mb-10 text-white">
                        ...
                        <h3 className="text-3xl font-black uppercase tracking-tighter">Configurar Proyecto</h3>
                    </div>
                    <form onSubmit={handleAddProject} className="space-y-10">
                        ...
                        <Button type="submit" className="w-full h-16 rounded-2xl font-black uppercase text-lg">Guardar Configuración</Button>
                    </form>
                </div>
        ...
        {activeTab === 'agents' && (
            <div className="p-10 space-y-10 animate-fade-in-up">
                 <form onSubmit={(e) => { e.preventDefault(); if(newUserName.trim()) { saveAgent(newUserName.trim()); setNewUserName(''); loadData(); toast.success(t.added); } }} className="flex gap-4 max-w-2xl mx-auto bg-slate-50 dark:bg-slate-850 p-6 rounded-3xl border border-slate-100 dark:border-slate-800">
                    <input value={newUserName} onChange={e => setNewUserName(e.target.value)} className="flex-1 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 outline-none focus:border-indigo-600 font-bold dark:text-white" placeholder="Nombre del Agente" />
                    <Button type="submit" className="h-[58px] px-10">Registrar</Button>
                 </form>
                 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {agents.map(a => (
                        <div key={a.id} className="p-6 bg-slate-50 dark:bg-slate-850 rounded-[2rem] border border-slate-100 dark:border-slate-800 flex flex-col items-center group">
                            ...
                            <span className="font-black text-xs uppercase dark:text-white">{a.name}</span>
                            <button onClick={() => { if(confirm("¿Eliminar agente?")) { deleteAgent(a.id); loadData(); } }} className="mt-4 text-[9px] font-black uppercase text-red-500 opacity-0 group-hover:opacity-100 transition-all">Eliminar</button>
                        </div>
                    ))}
                 </div>
            </div>
